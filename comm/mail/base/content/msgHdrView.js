  ...
  MsgHdrToMimeMessage(
    ...
    async function (msgHdr, mimeMsg) {
      try {
        ...
        window.openDialog( // = windows.arugmnt[1]
        ...
        // Suppress automatic quoting of the original body by providing a dummy "htmlToQuote"
        params.htmlToQuote = " "; 
        ...
        // Force explicit threading IDs since composeFields manual override drops the native message auto-parser
        ...
        try {
          ...
          msgCompose.sendMsg(Ci.nsIMsgCompDeliverMode.Now, identity, "", null).then(() => {
            ...
            // Fallback: If silent send fails, open the window so the user doesn't lose their work
            params.htmlToQuote = ""; // Restore default quoting for fallback window
            MailServices.compose.OpenComposeWindowWithParams(null, params);
          });
        } catch (ex) {
